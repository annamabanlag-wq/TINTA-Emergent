...
export default function ArtistApply() {
  ...
  return <ScrollView style={styles.root} contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
    ...
    <View style={styles.uploadCard}>
      ...
      <Pressable disabled={uploadingId || busy} onPress={pickGovernmentId} style={[styles.secondary, (uploadingId || busy) && styles.disabled]}><Text style={styles.secondaryText}>{uploadingId ? "UPLOADING ID..." : governmentIdPath ? "REPLACE GOVERNMENT ID" : "UPLOAD GOVERNMENT ID"}</Text></Pressable>
    ...
    <View style={styles.uploadCard}>
      ...
      <Pressable disabled={uploadingWork || busy} onPress={pickFinishedWork} style={[styles.cta, (uploadingWork || busy) && styles.disabled]}><Text style={styles.ctaText}>{uploadingWork ? "UPLOADING WORK..." : "UPLOAD FINISHED TATTOO WORK"}</Text></Pressable>
      <Text style={completedWorkPaths.length ? styles.success : styles.required}>{completedWorkPaths.length ? `✓ ${completedWorkPaths.length} FINISHED WORK PHOTO${completedWorkPaths.length === 1 ? "" : "S"} READY` : "AT LEAST 1 PHOTO REQUIRED"}</Text>
      {!!completedWorkPaths.length && token ? <View style={styles.workGrid}>{completedWorkPaths.map((path, index) => <View key={`${path}-${index}`} style={styles.workCard}>
...
const styles = StyleSheet.create({
  ...
  submitError: { color: colors.error, fontSize: 11, fontWeight: "900", letterSpacing: 1.2, lineHeight: 18 },
  submitNotice: { color: colors.onSurface, fontSize: 11, fontWeight: "900", letterSpacing: 1.2, lineHeight: 18 },
