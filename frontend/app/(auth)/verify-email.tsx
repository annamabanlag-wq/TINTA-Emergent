...
export default function VerifyEmail() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setMessage(isArtist ? "EMAIL VERIFIED. SIGN IN TO COMPLETE YOUR ARTIST PROFILE." : "EMAIL VERIFIED. YOU CAN NOW SIGN IN.");
      setTimeout(() => router.replace({ pathname: "/(auth)/sign-in", params: { next: isArtist ? "artist" : "" } }), 700);
  ...
  return (
    <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      <ScrollView contentContainerStyle={[styles.form, { paddingTop: insets.top + spacing.xl, paddingBottom: insets.bottom + spacing.xl }]} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.subtitle}>{isArtist ? "VERIFY YOUR ARTIST ACCOUNT" : "CHECK YOUR INBOX"}</Text>
